...
- Re-running the same track is a no-op at three levels: the m4a in `MUSIC_DIR` (skip download+convert), the library track found by `apple_music.find_track_id(title, artist)` (skip `add POSIX file`), and the playlist entry matched by persistent ID (skip `duplicate`) — but `move_to_top` still runs, so a re-sent track jumps to the top of the playlist
- New playlist entries go to the **top** of the playlist as a separate pipeline step: `duplicate` can only append, so `move_to_top` moves the entry to the end and then every other entry behind it (`move ... to end of thePlaylist`, n moves), which keeps the others in their old order. ~1s inside Music (~4s wall with osascript startup) per 150 entries, far below the 60s osascript timeout; `move` never removes anything, so a failure midway leaves the playlist rotated. The script then checks that entry 1 is the track and Python raises if not (Music ignores unsupported `move` locations silently). A process-wide lock keeps two concurrent pipelines from interleaving their playlist scripts
...
- Special chars in titles (& parentheses quotes) break AppleScript `whose name contains`. Persistent ID stays the primary handle; the only name-based lookup is `find_track_id`, which uses `whose name is` with quotes and backslashes escaped by `_escape()`
